...
export default function DetalhesProfessor({navigation, route}) {
  ...
  const buttonAlertDelete = () =>
    Alert.alert('Delete', 'Deseja deletar o professor?', [
      {
        text: 'cancelar',
      ...
      {text: 'Sim', onPress: async () => {
        ...
        try {
            await deletarUsuario(professor.key);
        } catch (error) {
            console.log(error);
    ...
    <Pressable onPress={Keyboard.dismiss} style={styles.container}>
      <ImageBackground
        ...
        <View style={styles.formContext}>
          ...
          <Text style={styles.input}>{professor.value.nome}</Text>

          <View style={styles.box}>
            <Text style={styles.textForm}>Email:</Text>
          </View>

          <Text style={styles.input}>{professor.value.email}</Text>

          <View style={styles.box}>
            <Text style={styles.textForm}>Telefone:</Text>
          </View>

          <Text style={styles.input}>{professor.value.telefone}</Text>

          <View style={styles.box}>
            <Text style={styles.textForm}>Departamento:</Text>
          </View>

          <Text style={styles.input}>{professor.value.nomeSetor}</Text>

          <View style={styles.navbar2}>
            <TouchableOpacity
              style={styles.b}
              onPress={() =>
                navigation.navigate('EditarProfessor', {item: professor})
              }>
